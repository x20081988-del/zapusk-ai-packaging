...
Last updated: 2026-08-26 (Sprint 65.P2: кнопки «Сделано» и «Разбери на шаги» на /crm).

---

## Completed (Sprint 65.P2 «Сделано» и «Разбери на шаги» 2026-08-26)

**Задача** — владелец на живом экране /crm: (1) «нет кнопки, что задача уже реализована» - «Закрыть» как «сделано» не считывается; (2) подсказка «Скажи "разбери" - распишу подзадачи» была обещанием без обработчика: ни кнопки, ни команды не существовало нигде (паттерн card-promise-needs-handler).

**Что сделано** — (1) кнопки закрытия переименованы: «Сделано» / «Сделано, закрыть» (готовый чек-лист), подтверждение тоже «Сделано»; механика прежняя (close_task с резолюцией). (2) Настоящая кнопка «Разбери на шаги» в плашке «Не разложена на шаги»: POST /api/crmweb/act с `kind: 'decompose'` (actSchema расширена enum-полем). Источник: crm_action получил колонку kind (do|decompose), decompose-бриф велит разложить задачу на 3-7 подзадач через founder_tasks.add_subtask (owner=agent/owner) и НЕ выполнять ее; проверка артефакта для decompose - подзадачи реально в реестре, иначе failed. Исполнитель тот же, что у «Сделать»: очередь живой сессии Claude, фолбэк headless claude с denylist отправки. Пока прогон живет, карточка пишет «Раскладываю задачу на шаги...»; по done доска перечитывается сама - чек-лист появляется без ручного «Обновить». Подсказка токен-страницы crm_web сокращена до «Не разложена на шаги» (обещание команды убрано).

**Verification** — server+web `tsc` pass, `npm run build` pass, `crm_action.py --selftest` 10/10, мост selftest 53/53. Живьем: кнопка на /crm создала прогон kind=decompose по задаче #424, статус и индикация проверены в браузере.

---

## Completed (Sprint 65.P1 «Фокус дня» на /decide 2026-08-26)

**Задача** — владелец: «сделай CRM, которой я действительно буду пользоваться». Диагноз: CRM жила за двумя кликами от /decide, а доска встречала стеной из 67 просрочек - владелец переставал открывать. Решение из трех частей, эта - витрина: /decide отвечает не только «что решить», но и «что двигать сегодня».

**Что сделано** — полоса `FocusStrip` на /decide между балансами и очередью: до 5 сделок, где сегодня ход владельца (просрочка/сегодня, ход owner, сортировка от самой горящей), до 5 горящих задач (heat over/today), хвост «еще N сделок ждут хода · без следующего шага: N», ссылки в /crm и /crm/board. Выборку считает ИСТОЧНИК: `crm_web.focus_view()` в telegram-agent (границы суток в UTC - шаги хранятся концом суток UTC), мост отдает `GET /crmweb/focus` (selftest 53), кокпит - `GET /api/crmweb/focus` через `relayGet` (снимок при спящем маке достается бесплатно; мутаций в полосе нет, глушить нечего). Полоса не смеет ронять разбор очереди: любая ошибка - полосы просто нет; пустой фокус рисуется явно («день чист»), а не пустотой. Вторые две части живут в telegram-agent: массовый триаж карточек (ops-director) и авто-пауза остывших `crm_freshness.py` из ежедневного `crm_pulse --auto`.

**Verification** — server+web `tsc` pass, `npm run build` pass, selftest моста 53/53, `crm_freshness --selftest` 10/10. Живьем локально: /api/crmweb/focus 200 с реальными данными, экран /decide проверен в браузере (чипы просрочки, ссылки, консоль чистая).
...
## Completed (Sprint 64.P3 снимки отчетов 2026-08-17)

**Задача** — владелец: «сделай то же самое для остальных разделов кокпита». Инвентаризация: от мака зависят только разделы «Мой день» через /api/reports/:name (Здоровье системы, Почта, Заявки и чаты, полоска балансов на /decide); админка платформы живет в Prisma на Render и снимков не требует; routes/crm.ts фронтом не используется (кандидат на удаление).

**Что сделано** — reports.ts сохраняет каждый успешный конверт снимком (`report-<name>`), при `unreachable` отдает его 200-м с НАРАЩЕННЫМ age_sec (возраст замера + время со снимка - «измерено N ч назад» остается правдой), `stale: true` и `fetched_at`. `fetched_at` есть ТОЛЬКО у серверного снимка - экраны по нему отличают «мак спит» от штатного stale-кэша самого моста. SnapshotBanner добавлен на /system, /mail, /inbound (самый свежий fetched_at из панелей); BalancesStrip работает от снимка без правок - возраст замера уже показывался. Мутаций на этих экранах нет, замораживать нечего.

**Verification** — server+web `tsc` pass, `npm run build` pass. Смоук локально: живые GET сеют report-* снимки (502 у mail/site-leads на первом заходе - это штатный прогрев отчета мостом, не дефект); с мертвым мостом balances отдал 200 со stale, age_sec наращен (2330с), fetched_at честный; отчет без снимка - 503. Экран /system проверен в браузере: баннер, панель из снимка с «2 ч назад», панели без снимков с прежней честной ошибкой.
...
## Completed (Sprint 64.P1 снимок очереди 2026-08-17)

**Задача** — владелец: «научи сайт показывать последний снимок очереди», когда мак недоступен (спит/офлайн/туннель). До этого экран /decide в таком состоянии показывал только ошибку.

**Что сделано** — сервер держит ОДИН снимок последнего удачного пакета в файле на постоянном диске (`server/src/lib/decideSnapshot.ts`, UPLOADS_DIR, наружу не раздается). `GET /api/decide`: каждый успешный ответ обновляет снимок; при `unreachable` отдается снимок с `stale: true` и `fetched_at` (200), без снимка — прежний честный 503. POST-действия при спящем маке не меняются (503). Фронт: баннер «Мак сейчас недоступен. Это снимок очереди на ЧЧ:ММ ДД.ММ» с «Проверить снова», кнопки решений в stale глушатся (плюс второй пояс в decide()), «Показать следующие» прячется. Правило «пакет не кешируется нигде» пересмотрено решением владельца: снимок отдается только при недоступном источнике, только с явной пометкой.

**Verification** — server+web `tsc` pass, `npm run build` pass. Живой смоук локально: живой GET сохраняет снимок; с мертвым мостом GET отдает 200 stale (20 карточек, fetched_at честный), POST 503, без файла снимка 503 source_unreachable; экран с баннером и погашенными кнопками проверен в браузере (снимок времени в поясе владельца).

---

## Completed (Sprint 63.P13 CRM целиком 2026-08-14)

**Задача** — владелец: «перенеси CRM на zapusk ai полностью, как она реализована в crm_web: проекты/задачи/карточки/воронки». Прогон supergoal в 7 фаз (`.supergoal/crm-zapusk-ai-tech-pre-ipo-SAFRNq`).

**Что сделано** — раздел CRM целиком: `/crm` «Проекты и задачи» (направления → проекты → задачи, чек-листы toggle_sub, закрытие с резолюцией, кнопка «Сделать» с запуском прогона агента, поллингом, вопросом-ответом, строка автономии), `/crm/board` канбан всех карточек (4 колонки card_bucket, полные карточки, действия по контракту _btn_disabled, DnD со сменой статуса, поиск, чипы стримов), `/crm/p/:slug` пять воронок сделок (этапные колонки + хвост Выиграно/Пауза/Отказ, DnD с реактивацией закрытых, модалка с историей/заметками/сменой этапа и статуса). Чипы-переходы CrmNav на всех экранах. Мост decide_bridge получил семейство `/crmweb/*` - тонкий passthrough к функциям crm_web (view и мутации не дублируются); задачи обогащаются прогонами; просмотры отмечаются в crm_views (бейджи «не открывал» честные). Старый простой экран /crm (Sprint 63.P12) удален - канбан его поглотил.

**Verification** — selftest моста 52/52; tsc server+web, build чисты; живьем: паритет счетчиков со crm_web до единицы (133 карточки, воронки 2/33/9/19/6, pm 10/40/14 + строка автономии дословно), toggle_sub/close_task/DnD/заметки/реактивация проверены на смоук-объектах по базе и вычищены; призрачные карточки не роняют доски; 401/403 гейты; мобильный 390px без горизонтального скролла. «Сделать» в смоуке живых прогонов не создавал.

---

## Completed (Sprint 63.P12 экран «CRM» 2026-08-14)

**Задача** — владелец: «перенеси мою /crm на zapusk ai в личный кабинет».

**Что сделано** — доска founder_crm (telegram-agent) в кабинете владельца. Мост decide_bridge получил `GET /crm` (карточки по стримам, счетчики нужен-шаг/просрочено/мяч-завис, флаги той же логикой, что CLI) и `POST /crm` (шаг/стадия/касание/статус/сделан/отложить); selftest вырос до 34 проверок. В кокпите: `server/src/routes/crm.ts` (прокси через lib/decideBridge, SUPER_ADMIN), `web/src/pages/Crm.tsx` (вкладки статусов, стримы, раскрывающиеся действия по карточке, редактор шага с «чей ход» и сроком), `web/src/lib/crm.ts`, пункт «CRM» в «Мой день», маршрут `/crm`. Источник правды не переезжал: экран ничего не хранит, каждое действие пишет в founder_crm тем же кодом, что CLI и бот, и попадает в crm_log.

**Verification** — server и web `tsc` pass, `npm run build` pass, `--selftest` 34/34, живой смоук: доска 120 карточек 5 стримов (включая consulting вне канонической четверки — не теряется), 401 без авторизации, 409 по несуществующей карточке, сквозная запись шага из UI проверена по crm_log и вычищена. QA-блокер закрыт: срок шага форматируется в UTC как в CLI/боте (локальная зона показывала дату на день позже и через input type=date сдвигала дедлайн в базе при каждой правке шага); notes с PII убраны из payload; ярлык founder; сброс стейта редактора после тихой перезагрузки.

---

## Completed (this sprint — Sprint 63.P11 аудит Codex: 12 находок, 12 закрыто 2026-08-13)

**Задача** — владелец: «отдай codex на аудит». Два прогона независимого ревью (агентская сторона и кокпит), все 12 находок с дословными якорями, 11 подтверждены + 1 частично, все закрыты.

**Кокпит** — (1) «Обновить» после «Показать следующие» сбрасывал очередь к 20 и прятал кнопку — теперь режим сохраняется; (2) заголовок финала при скрытых карточках стал честным «Показанное разобрано»; (3) POST принимает успехом только явный `ok:true`, небулев `ok` — 502; (4) пустой список при непустом `degraded` больше не пишет «Решений нет» рядом с баннером о недоступных видах — отдельное состояние «Показать нечего» с «Повторить»; (5) `no-store` на всех ответах картинки, не только успешных — 404 не кешируется навсегда; (6) загрузчик картинки уважает `VITE_API_BASE_URL` — раздельное размещение SPA и API не сломает превью.

**Агент** — приемка получила «Потом» (декодер умел, карточка не объявляла); удержанная гейтом почта не помечается показанной; контракт `ok` нормализован в `apply_decision` (пульт отдает карточку без `ok`); отсрочки приемок фильтруются до среза LIMIT; нечитаемое состояние канала попадает в `degraded`; светофор скиллов считает паритет Codex.

**Точность Codex на этом аудите** — 12 из 12 замечаний с дословными якорями, ни одной галлюцинации. Накопленная по журналу: 97%.

---

## Completed (Sprint 63.P10 правки по UX-экспертизе 2026-08-13)

**Задача** — владелец: «чини все» по итогам UX-экспертизы Fable (вердикт «терпимо», 20 находок в трех уровнях).

**Критичные (5 из 5 закрыты)** — (1) финал очереди: живой счетчик «Осталось N из M», карточка «Все разобрано» после последнего решения, кнопка «Показать следующие K» (мост научился `?limit=`, потолок 200); (2) полный текст решений вместо обрезки на полуслове + «развернуть» для длинных тел; (3) `renderInlineMarkup` на «Входящих» и «Здоровье системы» — звездочки больше не видны; (4) «Закрыть» через подтверждение: первый клик «Точно закрыть?», сброс через 4с; (5) почта честная: «30 важных, показаны 5», категории до двух ярлыков, призыв-обманка убран.

**Важные (8 из 8 закрыты)** — сайдбар в две секции «Мой день» / «Админка платформы»; «Входящие» переименованы в «Заявки и чаты», конверт отдан Почте; вердикты-светофоры на «Здоровье системы» («ИТОГ: все живо / N к решению»), простыня под «подробнее»; бейдж роли скрыт на мобиле — заголовок экрана вернулся; подсказка вида один раз под заголовком группы; жаргон переведен («была до запуска учета» вместо baseline, «приоритет 92» убран); даты Jivo в «11.08 09:53»; черновикам с вариантами добавлено пояснение.

**Косметика (6 из 7 закрыты)** — дубль фразы Авито; тест-заявки отфильтрованы; конверт и стрелки из бота убраны, 🔴 отклеен от слова; env-имена из текстов ошибок; промо-карточка скрыта у владельца; лимит Claude вернулся в полоску балансов. НЕ закрыто: тултип title недоступен с телефона (принято как есть — вся суть и так на экране).

**Verification** — server и web `tsc` pass, build + leak-check pass, `--selftest` 21/21, мобильный скриншот 390px: заголовок виден, подсказка одна, «развернуть» работает, вердикты цветные.

---

## Completed (Sprint 63.P9 экран «Входящие» 2026-08-13)
