...
class MockAILeadsProvider implements LeadProvider {
  ...
  private onboardingFor(mode: AILeadsMode, readiness: BriefingReadiness) {
    ...
    return {
      ...
      description: readiness.criticalReady
        ? 'Бриф готов. После согласования упаковки и юридической части мы запустим AI-каналы — звонки, мессенджеры, повторные касания.'
...
function buildStrategy(project: ProjectForAILeads | null): InvestorStrategy {
  ...
  return {
    ...
    keyTriggers: ['денежный поток', 'дивиденды', 'рост', 'pre-IPO', 'недвижимость', 'AI', 'экспорт', 'контроль рисков'],
    icpInvestors: [
      ...
      'партнёры, которым нужен понятный денежный поток и прозрачный контроль',
    ],
  };
}
...
function mockLeads(projectName: string): AILead[] {
  ...
  const seeds: Seed[] = [
    // ── Sprint 62.P3 demo showcase — 3 пронумерованных лида с реальными
    // записями AI-обзвона. Поставлены первыми (minutesAgo 1-3), чтобы на демо
    // founder в первую очередь видел свежий поток квалифицированных лидов с
    // прослушиваемыми записями и понятным контекстом.
    {
      id: 'lead-demo-1', status: 'HOT', minutesAgo: 1,
      name: 'Лид №1', phone: 'скрыто для Демо',
      check: 'от 1 млн ₽', horizon: 'готов к звонку специалиста', profile: 'AI-обзвон · подтверждённый интерес',
      tags: ['ЛИД №1', 'HOT', 'AI-ОБЗВОН'],
      summary: 'AI-агент квалифицировал инвестора по базовому скрипту. Подтвердил чек от 1 млн ₽ и готовность к звонку специалиста.',
      detail: 'Контакт получен через AI-обзвон базы. AI зафиксировал интерес, обсудил формат участия и согласовал звонок с фаундером. Слушайте запись разговора целиком: AI ведёт по скрипту квалификации, фиксирует все ключевые ответы.',
      nextStep: 'Передать менеджеру для назначения звонка с фаундером в течение 24 часов',
      sent: ['короткое резюме сделки', 'инвестиционное предложение'],
      recordingId: 'cd2e594f-de27-4358-aa33-f3026010057f',
      audioUrl: aicall('cd2e594f-de27-4358-aa33-f3026010057f'),
      durationSec: 230,
    },
    {
      id: 'lead-demo-2', status: 'HOT', minutesAgo: 2,
      name: 'Лид №2', phone: 'скрыто для Демо',
      check: 'от 1 млн ₽', horizon: '1-2 недели', profile: 'AI-обзвон · готов к материалам',
      tags: ['ЛИД №2', 'HOT', 'AI-ОБЗВОН'],
      summary: 'AI зафиксировал подтверждённый интерес и запрос на пакет материалов. Решение готов принять в течение 1-2 недель.',
      detail: 'Запись разговора показывает: инвестор слушает AI-питч до конца, задаёт встречные вопросы по экономике, явно сигнализирует готовность дальше. AI закрыл квалификацию и зафиксировал next-action на стороне команды.',
      nextStep: 'Отправить пакет материалов и согласовать Zoom с фаундером',
      sent: ['короткое резюме сделки', 'one-pager'],
      recordingId: 'd2a0bb2c-0001-4002-aa99-000000000002',
      durationSec: 235,
    },
    {
      id: 'lead-demo-3', status: 'WAITING', minutesAgo: 3,
      name: 'Лид №3', phone: 'скрыто для Демо',
      check: 'от 1 млн ₽', horizon: 'до 30 дней', profile: 'AI-обзвон · нужна доп. информация',
      tags: ['ЛИД №3', 'AI-ОБЗВОН', 'MATERIALS REQUESTED'],
      summary: 'AI зафиксировал интерес к проекту с запросом подробных материалов. Готовность к следующему шагу — после изучения пакета.',
      detail: 'Запись содержит короткий AI-питч и развёрнутый диалог про условия. AI корректно отработал вопросы инвестора и зафиксировал готовность вернуться к разговору после материалов. Передан в очередь специалиста.',
      nextStep: 'Отправить пакет материалов и поставить повторное касание через 3 дня',
      sent: ['презентация', 'инвестиционное предложение'],
      recordingId: 'd3a0bb2c-0001-4003-aa99-000000000003',
      durationSec: 90,
    },
    {
    ...
    },
    {
      ...
      nextStep: 'Отправить пакет материалов и повторное касание через 2 дня',
      sent: ['презентация', 'ванпейджер', 'план повторных касаний'],
...
function communicationsFor(seed: { id: string; minutesAgo: number; sent: string[] }, projectName: string, now: number): AILeadCommunication[] {
  ...
  return [
    ...
    },
    {
      ...
      title: 'Follow-up',
      body: 'AI зафиксировал следующий шаг и поставил напоминание на повторное касание.',
